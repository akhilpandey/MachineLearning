...
def CalcutaleRandomDS():
	...
	for feature in range(CONSTANT_F):
		for i in range(len(optimalDS[feature])):
			tmpRandom.append([feature, optimalDS[feature][i]])

	finalOptimalDS = choice(tmpRandom)
...
while(True):
	...
	getOptimalDS = CalcutaleRandomDS()
	fi = getOptimalDS[0]
	...
	if(NoOfRounds == 1):
		...
		if((abs(prevAUC - auc) < .00001) and auc > .97):
